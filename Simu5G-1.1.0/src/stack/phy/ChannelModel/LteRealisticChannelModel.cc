...
void LteRealisticChannelModel::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        oldTime_ = -1;
        oldRsrq_ = 0;
        sinrThreshold_ = par("sinrThreshold");
    }
}
...
std::vector<double> LteRealisticChannelModel::getSINR(LteAirFrame *frame, UserControlInfo* lteInfo)
{
    ...
    for (unsigned int i = 0; i < numBands_; i++)
    {
        ...
        //--- METRICS EMISSION ---
        double receivedPower = snrVector[i]; // this is received power in dBm before subtracting noise and interference
        double interferencePlusNoise = den;
        double finalSinr = receivedPower - interferencePlusNoise;
...
bool LteRealisticChannelModel::isError(LteAirFrame *frame, UserControlInfo* lteInfo)
{
   ...
   for (it = rbmap.begin(); it != rbmap.end(); ++it)
   {
       ...
       for (jt = it->second.begin(); jt != it->second.end(); ++jt)
       {
           ...
           // for statistic purposes
           double currentSinr = snrV[jt->first];
           sumSnr += currentSinr;
           usedRBs++;

           // --- Start of metrics calculation ---
           // Re-calculate Interference + Noise for this RB to get absolute power values
           double interferenceAndNoise_dBm;
           if (dir == DL)
           {
               // Get interference from other cells
               std::vector<double> multiCellInterference;
               multiCellInterference.resize(numBands_, 0);
               computeDownlinkInterference(lteInfo->getSourceId(), id, phy_->getCoord(), false, lteInfo->getCarrierFrequency(), rbmap, &multiCellInterference);

               // Get interference from external cells (if any)
               std::vector<double> extCellInterference;
               extCellInterference.resize(numBands_, 0);
               computeExtCellInterference(lteInfo->getSourceId(), id, phy_->getCoord(), false, lteInfo->getCarrierFrequency(), &extCellInterference);

               // Total Noise (linear)
               double totN_linear = dBmToLinear(thermalNoise_ + ueNoiseFigure_);

               // Total Interference + Noise for this specific band (linear)
               double totalInterferenceAndNoise_linear = extCellInterference[jt->first] + totN_linear + multiCellInterference[jt->first];
               interferenceAndNoise_dBm = linearToDBm(totalInterferenceAndNoise_linear);
           }
           else // Uplink
           {
               // Get UL interference
                std::vector<double> ulInterference;
                ulInterference.resize(numBands_, 0);
                computeUplinkInterference(lteInfo->getDestId(), id, false, lteInfo->getCarrierFrequency(), rbmap, &ulInterference);

                // Total Noise (linear)
                double totN_linear = dBmToLinear(thermalNoise_ + bsNoiseFigure_);

                // Total Interference + Noise for this specific band (linear)
                double totalInterferenceAndNoise_linear = ulInterference[jt->first] + totN_linear;
                interferenceAndNoise_dBm = linearToDBm(totalInterferenceAndNoise_linear);
           }

            // Received Power (dBm) = SINR (dB) + (I+N) (dBm)
           double receivedPower_dBm = currentSinr + interferenceAndNoise_dBm;
           sumReceivedPower += receivedPower_dBm;
           sumInterference += interferenceAndNoise_dBm;
   ...
   // emit SINR statistic - THIS IS NOW DONE IN getSINR
   // if (usedRBs > 0)
