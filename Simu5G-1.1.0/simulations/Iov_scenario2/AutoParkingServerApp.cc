...
void AutoParkingServerApp::loadParkingAreas()
{
    try {
        // 尝试以相对路径和绝对路径加载文件
        std::string filename = parkingAreasFile;
        std::ifstream file(filename.c_str());
        if (!file.is_open()) {
            filename = getParentModule()->getFullPath() + "/" + parkingAreasFile;
            file.open(filename.c_str());
        }
        
        if (!file.is_open()) {
            throw cRuntimeError("Could not open parking areas file %s", parkingAreasFile.c_str());
        }
        ...
        // 解析XML
        cXMLElement* root = xmlParse(buffer.str().c_str());
        if (!root) {
            throw cRuntimeError("Could not parse parking areas XML file");
        }
...
void AutoParkingServerApp::processParkingRequest(Packet *packet)
{
    try {
        ...
        if (parkingArea) {
            ...
            // 发送给车辆
            socket.sendTo(command, destAddr, destPort);
            numRelayedCommands++;
